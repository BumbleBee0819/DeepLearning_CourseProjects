...
class RegularizedLinearRegressor_Multi:
    ...
    def train(self,X,y,reg=1e-5,num_iters=100):
        ...
        self.theta = theta = scipy.optimize.fmin_bfgs(self.loss, theta, fprime = self.grad_loss, args=(X,y,reg),maxiter=num_iters)
    ...
    def normal_equation(self,X,y,reg):
        ...
        self.theta = theta_n = np.linalg.inv(X.T.dot(X)).dot(X.T.dot(y))
        ###########################################################################
        return theta_n
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        J = np.sum((X.dot(theta)-y)**2)/2./num_examples
        J += reg/2./num_examples*theta[1:].dot(theta[1:])
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        grad = X.T.dot(X.dot(theta)-y)/(num_examples*1.0)
        grad[1:] += reg*(theta[1:])/(num_examples*1.0)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
