...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        J = -np.dot(np.log(utils.sigmoid(np.dot(X, theta))), y) - np.dot(np.log(1 - utils.sigmoid(np.dot(X, theta))), 1 - y)
        J = J / m
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
    ...
    def grad_loss(self, *args):
        ...
        ##########################################################################
        grad = np.dot((utils.sigmoid(np.dot(X, theta)) -y).T, X) / m
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        # TODO: 1-2 lines of code expected                                       #
        ##########################################################################
        J = -np.dot(np.log(utils.sigmoid(np.dot(X, theta))), y) - \
            np.dot(np.log(1 - utils.sigmoid(np.dot(X, theta))), 1 - y) + reg / 2. * theta[1:].dot(theta[1:])
        J = J / m
